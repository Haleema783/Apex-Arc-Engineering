...
const nav: { href: string; label: string; id: string; route?: boolean }[] = [
  ...
  { href: "/services", label: "Services", id: "services", route: true },
      ...
      <div className="container flex h-16 sm:h-20 items-center justify-between gap-3">
        ...
        <nav className="hidden lg:flex items-center gap-1">
          {nav.map((n) => {
            ...
            return n.route ? (
              <Link key={n.href} to={n.href} className={classes} aria-current={isActive ? "page" : undefined}>
                {inner}
              </Link>
            ) : (
              <a
        ...
        <div className="lg:hidden border-t bg-background/95 backdrop-blur-lg animate-fade-in">
          <nav className="container py-3 flex flex-col">
            {nav.map((n) => {
              ...
              return n.route ? (
                <Link key={n.href} to={n.href} onClick={() => setOpen(false)} className={classes}>
                  {n.label}
                </Link>
              ) : (
                <a
