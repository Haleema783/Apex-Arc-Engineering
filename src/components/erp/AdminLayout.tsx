...
import {
  ...
  Receipt, Percent, UserCog, Send, Wallet, History, ClipboardList, BarChart3, ScrollText, Menu,
} from "lucide-react";
...
const SidebarBody = ({ onNavigate, onSignOut, email }: { onNavigate?: () => void; onSignOut: () => void; email?: string | null }) => (
  <div className="flex h-full flex-col bg-sidebar text-sidebar-foreground">
    <Link to="/admin" onClick={onNavigate} className="flex items-center gap-2 px-6 h-16 border-b border-sidebar-border">
    ...
    <nav className="flex-1 p-3 space-y-1 overflow-y-auto">
      ...
      <div className="pt-3 mt-3 border-t border-sidebar-border">
        ...
        {adminOnlyItems.map((it) => (
          <NavLink key={it.to} to={it.to} onClick={onNavigate} className={navLinkClass}>
            <it.icon className="h-4 w-4" /> {it.label}
          </NavLink>
        ))}
      </div>
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <header className="md:hidden h-14 border-b bg-card flex items-center justify-between px-3 sticky top-0 z-30">
          <div className="flex items-center gap-2">
            ...
            <Link to="/admin" className="font-display font-bold text-primary text-sm">Apex Arc · Admin</Link>
          </div>
